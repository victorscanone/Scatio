...
public class MatrizState implements InterfaceAnimalState, Serializable {
    @Override
    ...
    public InterfaceAnimalState tentarMudarEstado(LocalDate nascimento, char sexo) {
        ...
        if (idade < 5) {
            ...
            Printes.espaco();
            System.out.println("--> SEU ANIMAL CONTINUA COMO UMA MATRIZ POR TER MAIS DE 10 ANOS E SER FÊMEA.");
        } else if (idade >= 10 && sexo == 'm') {
            retorno = new ReprodutorState();
            Printes.espaco();
            System.out.println("--> SEU ANIMAL AGORA ESTÁ NO ESTADO REPRODUTOR POR TER MAIS DE 10 ANOS E SER MACHO.");
        }
        return retorno;
    }

...
public class Ave implements InterfaceAnimal, Serializable {
    private String nome;
    ...
    public void setNome(String nome, String sitio) {
        boolean certo = false;
        File diretorio = new File(String.format("src\\main\\java\\sitios\\%s\\aves", sitio));
        ...
        if (certo) {
            Printes.espaco();
            System.out.println("--> JÁ EXISTE UMA AVE COM ESSE NOME.");
        } else {
            Printes.espaco();
            this.nome = nome;
        }
    ...
    public String getEstado() {
        if (estado == null) {
            ...
            return String.format("%s ave",estado.toString());
        }
    }
    ...
    public boolean serializar(String sitio) {
        if (!this.nome.equals("")) {
            try {
                ObjectOutputStream objectOutput = new ObjectOutputStream(new FileOutputStream(String.format("src\\main\\java\\sitios\\%s\\aves\\%s.byte", sitio, this.nome)));
                objectOutput.writeObject(this);
                objectOutput.close();
                Printes.espaco();
                System.out.println("--> AVE SALVA.");
                return true;
            } catch (IOException e) {
                Printes.espaco();
                System.out.println("--> PROBLEMA AO SERIALIZAR A AVE.");
                return false;
            }
        } else {
            Printes.espaco();
            System.out.println("--> COLOQUE UM NOME NA AVE ANTES DE SALVAR.");
            return false;
        }
    }
    ...
    public void verificarTransicao() {
        if (nascimento == null || estado == null) {
            Printes.espaco();
            System.out.println("--> PRIMEIRO DEFINA A DATA DE NASCIMENTO E O ESTADO DA AVE.");
        } else {

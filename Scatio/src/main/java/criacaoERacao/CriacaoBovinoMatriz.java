...
public class CriacaoBovinoMatriz implements InterfaceCriacaoMatriz, Serializable {
    ...
    public void setSetor(String setor, String sitio) {
        boolean certo = false;
        File diretorio = new File(String.format("src\\main\\java\\sitios\\%s\\criacoesBovinos", sitio));
        ...
        if (certo) {
            Printes.espaco();
            System.out.println("--> JÁ EXISTE UMA CRIAÇÃO BOVINA COM ESSE NOME.");
        } else {
            Printes.espaco();
            this.setor = setor;
        }
    ...
    public void adicionarNaCriacao(InterfaceAnimal animal) {
        ...
        if (animal.pegarEstado() instanceof MatrizState || animal.pegarEstado() instanceof FilhoteState) {
            animais.add(animal);
        } else {
            Printes.espaco();
            System.out.println("--> ESSE ANIMAL NÃO É UMA MATRIZ OU FILHOTE PARA FAZER PARTE DESSA CRIAÇÃO.");
        }
    ...
    public boolean serializar(String sitio) {
        if (!this.getSetor().equals("")) {
            try {
                ObjectOutputStream objectOutput = new ObjectOutputStream(new FileOutputStream(String.format("src\\main\\java\\sitios\\%s\\criacoesBovinos\\%s.byte", sitio, this.setor)));
